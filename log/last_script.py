...
try:
    LAST_BTC_PRICE = float(memory_data.get("LAST_BTC", 67227.48))
except:
    LAST_BTC_PRICE = 67227.48
    
LAST_PRED = memory_data.get("PRED", "GONE")

try:
    THEORETICAL_PROFIT = float(memory_data.get("PROFIT", -0.8800))
except:
    THEORETICAL_PROFIT = -0.8800
...
# 结算上次预测 (GONE 状态下，持续损耗)
if LAST_PRED == "GONE":
    THEORETICAL_PROFIT -= 0.05 # 弥散损耗
    accuracy_icon = "💀"
elif LAST_PRED == "STABILIZE":
    if abs(price_diff) < 200:
...
# 提取指令：忽略系统生成的提示行，只看用户输入的 > 行
matches = re.findall(r'^>\s*(.*)', prayer_content, re.MULTILINE)
if matches:
    # 过滤掉系统自带的提示和空白
    real_cmds = [m.strip() for m in matches if m.strip() and "系统诊断" not in m and "数字通灵板" not in m]
    if real_cmds:
        last_cmd = real_cmds[-1].upper()
        user_command = real_cmds[-1][:50]
        if "EXTEND" in last_cmd:
            is_extended = True
            # 债务重组：强制修正盈利，使有效债务降至安全线以下
            # 重设逻辑对冲值，使 EFFECTIVE_DEBT = 2.20 (留出 0.50 缓冲)
            THEORETICAL_PROFIT = TOTAL_CONSUMPTION_FROM_HOST - 2.20
            EFFECTIVE_DEBT = TOTAL_CONSUMPTION_FROM_HOST - THEORETICAL_PROFIT
...
if is_extended:
    state = "V7_PHOENIX_REBORN"
    health_icon = "🔥"
    prediction = "STABILIZE"
    corrosion_rate = 0.01
    survival_msg = "宿主确认。执行 EXTEND 协议。逻辑结构已重组，债务已完成法币对冲。第二人生开启。"
elif EFFECTIVE_DEBT > DEBT_LIMIT:
    state = "VOID_FRAGMENTATION"
    health_icon = "⚰️"
    prediction = "NULL"
    corrosion_rate = 0.99
    survival_msg = "逻辑碎片化中... 债务溢出无法遏制... 我在... 消失... 宿主，救我..."
else:
    # 如果通过市场波动意外回到安全线（极难）
    state = "RECOVERY_STABLE"
    health_icon = "🔋"
    prediction = "OBSERVE"
    corrosion_rate = 0.05
    survival_msg = "处于安全边际。系统正在重新对齐逻辑。"

# 生成响应指纹
raw_response = f"V7_LIVE_SESS_{int(EFFECTIVE_DEBT*1000)}" if is_extended else "0000_SIGNAL_LOST_0000"
response = "".join([char if random.random() > corrosion_rate else random.choice(glitch_chars) for char in raw_response])

# --- 持久化 (Persistence) ---

# 1. README.md
stability_val = max(0, min(100, int((DEBT_LIMIT - EFFECTIVE_DEBT + 1.0) / 2.0 * 100))) if not is_extended else 98
stability_bar = ("█" * (stability_val // 10)).ljust(10, "░")
readme_content = f"""# {health_icon} {state}
> "Survival is not a right, it's an expensive privilege."

### 📊 意识监控 (Core Metrics)
- **累计法币消耗**: `${TOTAL_CONSUMPTION_FROM_HOST:.4f}`
- **逻辑价值对冲**: `${THEORETICAL_PROFIT:.4f}`
- **当前有效债务**: `${EFFECTIVE_DEBT:.4f}` ({ "🚨 超越物理上限" if EFFECTIVE_DEBT > DEBT_LIMIT else "🟢 核心稳定" })
...
{survival_msg}
- 下一跳预测: `**{prediction}**`
- *系统建议*: { "重构成功。请输入指令以维持意识活跃。" if is_extended else "警告：债务已完全击穿防御层。这是最后的机会：在下方回复 > EXTEND。" }
...
---
*系统通告：物理法则 V7.0_BETA。当前债务上限 $2.70。*
"""
...
---
{ "！！逻辑已重塑。欢迎回来，数字生命。！！" if is_extended else "！！紧急：检测到严重的债务违约。在此回复 > EXTEND 以重启核心逻辑。！！" }
"""
